	...
	<form id="openai-form" action="" method="GET" enctype="multipart/form-data" class="mt-24">		
		...
		<div class="row justify-content-md-center">	
			<div class="col-sm-12 text-center">
				<h3 class="card-title fs-20 mb-3 super-strong"><i class="fa-solid fa-image-landscape mr-2 text-primary"></i><?php echo e(__('Chat Image')); ?></h3>
				<h6 class="mb-0 fs-12 text-muted"><?php echo e(__($chat->description)); ?></h6>
				<div class="mb-4" id="balance-status">
					<span class="fs-11 text-muted pl-3"><i class="fa-sharp fa-solid fa-bolt-lightning mr-2 text-primary"></i><?php echo e(__('Your Balance is')); ?> <span class="font-weight-semibold" id="balance-number"><?php if(auth()->user()->available_dalle_images == -1): ?> <?php echo e(__('Unlimited')); ?> <?php else: ?> <?php echo e(number_format(auth()->user()->available_dalle_images + auth()->user()->available_dalle_images_prepaid)); ?><?php endif; ?></span> <?php echo e(__('Dalle Images')); ?></span>
				</div>	
			</div>
...
unset($__errorArgs, $__bag); ?>" rows="1" id="message" name="message" placeholder="<?php echo e(__('Describe your image details here...')); ?>"></textarea>
...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		if (check_messages == 0) {
			...
			$.ajax({
				...
				method: 'POST',
				url: '/app/user/chat/image/conversation',
					...
					if (data == 'success') {
						...
						$('.chat-sidebar-messages').prepend(`<div class="chat-sidebar-message selected-message" id=${id}>
								<div class="chat-title" id="title-${id}">
									<?php echo e(__('Chat Image Conversation')); ?>
	...
	$(document).on('click', ".chat-sidebar-message", function (e) { 
		...
		$.ajax({
				headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
				method: 'POST',
				url: '/app/user/chat/image/history',
		...
		Swal.fire({
			...
			if (result.value) {
				...
				$.ajax({
					headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
					method: 'post',
					url: '/app/user/chat/image/rename',
					data: formData,
					processData: false,
					contentType: false,
					success: function (data) {
		...
		Swal.fire({
			...
			if (result.isConfirmed) {
				...
				$.ajax({
					headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
					method: 'post',
					url: '/app/user/chat/image/delete',
					data: formData,
					processData: false,
					contentType: false,
					success: function (data) {
	...
	function process(message) {
		...
		$.ajax({
			headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
			method: 'POST',
			url: '/app/user/chat/image/process',
			data: formData,
			contentType: false,
			processData: false,
			cache: false,
			beforeSend: function() {  
				let notification = '<?php echo e(__("Generating your image ")); ?>' + '<div class="spinner-border spinner-border-sm" role="status"><span class="visually-hidden">Loading...</span></div>';
			...
			success: function (data) {		
				...
				if (data['status'] == 'success') {		
					...
					toastr.error('<?php echo e(__('Invalid image detail request, contact support')); ?>');
					msgerSendBtn.disabled = false	
				}
			},
			error: function(data) {
	...
	function clearConversation() {
		...
		fetch('/app/user/chat/image/clear', {
				headers: {
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/viralhub/public_html/nexgen.to/resources/views/default/user/chat_image/index.blade.php ENDPATH**/ ?>