	...
	<form id="openai-form" action="" method="GET" enctype="multipart/form-data" class="mt-24">		
		...
		<div class="row justify-content-md-center">	
			<div class="col-sm-12 text-center">
				<h3 class="card-title fs-20 mb-3 super-strong"><i class="fa-solid fa-globe mr-2 text-primary"></i><?php echo e(__('AI Web Chat')); ?></h3>
...
<script type="text/javascript">
	...
	$(document).on('click', ".chat-sidebar-message", function (e) { 
		...
		$.ajax({
			...
			method: 'POST',
			url: '/app/user/chat/web/conversation',
	...
	function process(message) {
		...
		fetch('/app/user/chat/web/process', {
				headers: {
	...
	function analyze(url) {
		...
		$.ajax({
			headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
			method: 'post',
			url: '/app/user/chat/web/embedding',
			data: formData,
			processData: false,
			contentType: false,
			success: function (data) {
				console.log(data)
				if (data['status'] == 'success') {
					toastr.success('<?php echo e(__('Web URL has been analyzed')); ?>');
					new_chat_id = data['id'];
					$('#progress-drop-box').addClass('hidden');
					$('#intro-drop-box').removeClass('block-display');
	...
	function checkBalance(task, message) {
		...
		$.ajax({
			headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
			method: 'post',
			url: '/app/user/chat/web/check-balance',
			data: formData,
			processData: false,
			contentType: false,
			success: function (data) {
	...
	function createConversationBox(new_chat_id) {
		...
		$.ajax({
			headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
			method: 'post',
			url: '/app/user/chat/web/metainfo',
			data: formData,
			processData: false,
			contentType: false,
			success: function (data) {
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/viralhub/public_html/nexgen.to/resources/views/default/user/chat_web/index.blade.php ENDPATH**/ ?>